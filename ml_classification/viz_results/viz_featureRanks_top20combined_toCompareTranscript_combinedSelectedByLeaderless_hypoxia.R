...
###### leadered
hypoxia_leadered_featureSort <- read.table('../ml_features_halfLifeClass/HalfLifeCls_hypoxia_combinedSelected_leadered_byLeaderlessHypoxia_jointlyByLeaderedLeaderless_feature.txt',
...
###### leaderless
hypoxia_leaderless_featureSort <- read.table('../ml_features_halfLifeClass/HalfLifeCls_hypoxia_combinedSelected_leaderless_byLeaderlessHypoxia_jointlyByLeaderedLeaderless_feature.txt',
                                             col.names = c("feature", "Gini_mean", "Gini_sd"))
...
feature_toCompareTop20 <- bind_rows(hypoxia_leadered_featureSort_type[1:20, ], hypoxia_leaderless_featureSort_type[1:20, ])
...
feature_toCompareTop20_unique_withRanking_p1 <- feature_toCompareTop20_unique %>% 
  ...
  rename(hypoxia_leadered_ranking = ranking) %>% 
  mutate(type_p1 = coalesce(type.x, type.y)) %>% 
  left_join(hypoxia_leaderless_featureSort_type, by = "feature") %>%
  rename(hypoxia_leaderless_ranking = ranking)  %>% 
  select(feature, type, hypoxia_leadered_ranking, hypoxia_leaderless_ranking) %>%
  mutate(type = factor(type, levels = c("other", "Translation", "Codon", "CDSmfe", "CDSnucleotide"))) %>%
  arrange(desc(type), desc(feature))

##### added fake features in other models to match the number of feature here for hypoxia
feature_toCompareTop20_unique_withRanking <- feature_toCompareTop20_unique_withRanking_p1 %>% 
  ...
  mutate(condition = factor(condition, levels = c("hypoxia_leaderless_ranking", "hypoxia_leadered_ranking")))
...
ggplot(feature_toCompareTop20_unique_withRanking, aes(feature, condition)) + 
  ...
  coord_equal()
ggsave("./viz_featureRanks_toCompareTranscript/featureRankingTop20combined_toCompareTranscript_HalfLifeCls_hypoxia_combinedSelectedByLeaderlessHypoxia_jointlyByLeaderedLeaderless_withYLabels.png", width = 70, height = 20, units = "cm", dpi = 600)
...
ggplot(feature_toCompareTop20_unique_withRanking, aes(feature, condition)) + 
  ...
  theme(
    ...
    panel.grid = element_blank()) +
  coord_equal()
ggsave("./viz_featureRanks_toCompareTranscript/featureRankingTop20combined_toCompareTranscript_HalfLifeCls_hypoxia_combinedSelectedByLeaderlessHypoxia_jointlyByLeaderedLeaderless.png", width = 70, height = 20, units = "cm", dpi = 600)
