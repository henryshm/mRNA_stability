...
###### log phase
logPhase_leaderless_featureSort <- read.table('../ml_features_halfLifeClass/HalfLifeCls_logPhase_combinedSelected_leaderless_byLeaderlessLogPhase_jointlyByLogPhaseHypoxia_feature.txt',
...
###### hypoxia
hypoxia_leaderless_featureSort <- read.table('../ml_features_halfLifeClass/HalfLifeCls_hypoxia_combinedSelected_leaderless_byLeaderlessLogPhase_jointlyByLogPhaseHypoxia_feature.txt',
                                             col.names = c("feature", "Gini_mean", "Gini_sd"))
hypoxia_leaderless_featureSort_type <- hypoxia_leaderless_featureSort %>% 
  inner_join(feature_completeList, by = "feature") %>% 
  arrange(desc(Gini_mean)) %>% 
  mutate(ranking_p1 = 1:n()) %>% 
  mutate(ranking = (ranking_p1 / nrow(hypoxia_leaderless_featureSort)) * 100) %>% 
  select(-Gini_mean, -Gini_sd, -ranking_p1)

###### fold change in hypoxia
hypoxia2LogPhase_leaderless_featureSort <- read.table('../ml_features_halfLifeFcClass/HalfLifeFcCls_hypoxiaToLogPhase_combinedSelected_leaderless_byLeaderlessLogPhase_jointlyByLogPhaseFoldChange_feature.txt',
                                                      col.names = c("feature", "Gini_mean", "Gini_sd"))

hypoxia2LogPhase_leaderless_featureSort_type <- hypoxia2LogPhase_leaderless_featureSort %>% 
  inner_join(feature_completeList, by = "feature") %>% 
  arrange(desc(Gini_mean)) %>% 
  mutate(ranking_p1 = 1:n()) %>% 
  mutate(ranking = (ranking_p1 / nrow(hypoxia2LogPhase_leaderless_featureSort)) * 100) %>% 
  select(-Gini_mean, -Gini_sd, -ranking_p1)

####### combine important features
###### combined with top 20 features (vertical figures)
feature_toCompareTop20 <- bind_rows(logPhase_leaderless_featureSort_type[1:20, ], hypoxia_leaderless_featureSort_type[1:20, ], hypoxia2LogPhase_leaderless_featureSort_type[1:20, ])
...
feature_toCompareTop20_unique_initialAbundance <- logPhase_leaderless_featureSort_type %>% 
  ...
  inner_join(hypoxia_leaderless_featureSort_type, by = "feature") %>% 
  rename(hypoxia_ranking = ranking) %>% 
  mutate(feature = "initialAbundance_logPhase") %>% 
  inner_join(hypoxia2LogPhase_leaderless_featureSort_type, by = "feature") %>% 
...
feature_toCompareTop20_unique_withRanking_p1 <- feature_toCompareTop20_unique %>% 
  ...
  left_join(hypoxia_leaderless_featureSort_type, by = "feature") %>%
  rename(hypoxia_ranking = ranking) %>% 
  mutate(type_p2 = coalesce(type_p1, type)) %>% 
  left_join(hypoxia2LogPhase_leaderless_featureSort_type, by = "feature") %>% 
  rename(hypoxia2LogPhase_ranking = ranking) %>% 
  select(feature, type_p2, logPhase_ranking, hypoxia_ranking, hypoxia2LogPhase_ranking) %>% 
  rename(type = type_p2)

##### In leadered transcript code, if necessary, added fake features in other models to match the number of feature here for leaderless
...
ggplot(feature_toCompareTop20_unique_withRanking, aes(feature, condition)) + 
  ...
  coord_equal()
ggsave("./viz_featureRanks_toCompareCondition/featureRankingTop20combined_toCompareCondition_combinedSelectedByLogPhase_leaderless_jointlyByLogPhaseHypoxiaFoldChange.png", width = 70, height = 20, units = "cm", dpi = 600)
