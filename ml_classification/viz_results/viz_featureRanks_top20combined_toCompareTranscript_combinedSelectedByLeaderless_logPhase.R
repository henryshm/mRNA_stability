...
feature_toCompareTop20 <- bind_rows(logPhase_leadered_featureSort_type[1:20, ], logPhase_leaderless_featureSort_type[1:20, ])
...
feature_toCompareTop20_unique_withRanking_p2 <- data.frame(feature = c("initialAbundance_logPhase_fake1", "initialAbundance_logPhase_fake2", 
                                                                       "initialAbundance_logPhase_fake3"),
                                                           type = "other", logPhase_leadered_ranking = 3.2786885,
                                                           logPhase_leaderless_ranking = 8.6065574)

feature_toCompareTop20_unique_withRanking_p3 <- rbind(feature_toCompareTop20_unique_withRanking_p1, feature_toCompareTop20_unique_withRanking_p2)

feature_toCompareTop20_unique_withRanking <- feature_toCompareTop20_unique_withRanking_p3 %>%
...
ggplot(feature_toCompareTop20_unique_withRanking, aes(feature, condition)) + 
  ...
  coord_equal()
ggsave("./viz_featureRanks_toCompareTranscript/featureRankingTop20combined_toCompareTranscript_HalfLifeCls_logPhase_combinedSelectedByLeaderlessLogPhase_jointlyByLeaderedLeaderless_withYLabels.png", width = 70, height = 20, units = "cm", dpi = 600)
...
ggplot(feature_toCompareTop20_unique_withRanking, aes(feature, condition)) + 
  ...
  theme(
    ...
    panel.grid = element_blank()) +
  coord_equal()
ggsave("./viz_featureRanks_toCompareTranscript/featureRankingTop20combined_toCompareTranscript_HalfLifeCls_logPhase_combinedSelectedByLeaderlessLogPhase_jointlyByLeaderedLeaderless.png", width = 70, height = 20, units = "cm", dpi = 600)
